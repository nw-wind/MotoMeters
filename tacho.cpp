...
// Нижняя мигалка
int lowBlinkStart=2; // начинать мигать
int lowBlinkStop=4;  // прекращать мигать

// Верхняя мигалка
int hightBlinkStart=diods-4; // начинать мигать
...
void loop() {
  ...
  for (int i = 0; i < blocks; i++) {
    ...
    lc.setRow(0, i, (0xFF << shift) & 0xFF); // зажигаем 8-shift диобов в строчке.
  }
  
  // Здесь мигаем, когда надо
  // мигалки включены в блок ЗА основными диодами7 то есть, 18 диодов = 16 индикатор + 2 мигалки
  
  // Если светятся 2-4 диода, то мигаем раз в 1/4 секунды
  if (lowBlinkStart < d && d <= lowBlinkStop && blink.Now()) {
    if (bl) {
      lc.setLed(0, blocks, 0, HIGH);
      bl = 0;
    } else {
      lc.setLed(0, blocks, 0, LOW);
      bl = 1;
    }
  ...
  if (d >= highBlinkStart && blink.Now()) {
    if (!bh) {
      lc.setLed(0, blocks, 1, HIGH);
      bh = 1;
    } else {
      lc.setLed(0, blocks, 1, LOW);
      bh = 0;
    }
  } else { // Выключаем мигание
    lc.setLed(0, blocks, 1, LOW);
